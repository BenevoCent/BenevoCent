            ...
            .then(res => {
                if (res[0]) {
                    this.setState({
                        ...
                        selectedMonthDonation:
                            res[res.length - 1].monthlyDonation,
                    })
                } else {
                    this.setState({
                        monthlyDonations: [
                            {
                                month: `${date.getYear() +
                                    1900}-${date.getMonth() + 1}`,
                                monthlyDonation: 0,
                            },
                        ],
                        selectedMonthName: `${date.getYear() +
                            1900}-${date.getMonth() + 1}`,
                        selectedMonthDonation: 0,
                    })
                }
            ...
            <div style={{ marginTop: '1rem' }}>
                <div style={{ width: '100vw' }}>
                    <RaisedButton
                        ...
                        label={
                            monthInWords(this.state.selectedMonthName) !==
                            'undefined 0'
                                ? monthInWords(this.state.selectedMonthName)
                                : 'Loading...'
                        }
                    ...
                    <Menu>
                        {this.state.monthlyDonations.map(elem => {
                            ...
                            return (
                                <MenuItem
                                    key={elem.month}
                                    primaryText={monthInWords(elem.month)}
                                    onClick={() => {
                                        this.selectMonth(
                                            elem.month,
                                            elem.monthlyDonation,
                                        )
                                        this.getPlots(
                                            elem.month,
                                            this.props.user.uid,
